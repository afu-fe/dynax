...
import licenseFn from 'rollup-plugin-license'
  ...
  return licenseFn({
    thirdParty(dependencies) {
      ...
      function sortLicenses(licenses: Set<string>) {
        ...
        licenses.forEach((_license) => {
          if (/^\(/.test(_license)) {
            withParenthesis.push(_license)
          } else {
            noParenthesis.push(_license)
          }
        })
      ...
      const _licenses = new Set<string>()
        ...
        .map(
          ({
            ...
            for (const person of [author, ...maintainers, ...contributors]) {
              const _name = typeof person === 'string' ? person : person?.name
              if (_name) {
                names.add(_name)
              }
            }
